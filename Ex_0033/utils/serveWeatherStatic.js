...
async function serveWeatherStatic(req, res, baseDir) {
    const dateRes = new Date();
    const reqResName = req.url.slice(1);
    const pathToResource = path.join(baseDir, 'public', reqResName === '' ? 'indexWeather.html' : reqResName);
