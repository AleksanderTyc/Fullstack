...
async function getModelById(id: number | string): Promise<Model> {
    const searchedModel: Model | undefined = testModels.find((model: Model): boolean => model.id.toString() === id.toString());
    if (searchedModel === undefined) {
        throw new Error(`* E * Model id ${id} not found`);
    }
